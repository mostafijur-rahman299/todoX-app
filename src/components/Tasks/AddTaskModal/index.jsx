...
import { generateId, getNextFreeSlot } from "@/utils/gnFunc";
...
const AddTaskModal = ({ visible, onClose }) => {
	...
	useEffect(() => {
				...
				if (!storedCategories) {
					...
					if (nonExistingCategories.length > 0) {
						await AsyncStorage.setItem(
							"categories",
							JSON.stringify(nonExistingCategories),
						);
		...
		const loadTasks = async () => {
			try {
				const storedTasks =
					(await getDataLocalStorage("task_list")) || [];
	...
	const saveTask = async () => {
		...
		try {
			let { date, startTime, endTime } = newTask.date
				? {
						date: newTask.date,
						startTime: newTask.startTime || null,
						endTime: newTask.endTime || null,
				  }
				: getNextFreeSlot(tasks, newTask.date);
