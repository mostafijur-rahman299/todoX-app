...
const Upcoming = () => {
    ...
    const priorityOptions = [
        { label: 'Priority 1', value: 'priority1', color: colors.error, icon: 'flag' },
        { label: 'Priority 3', value: 'priority3', color: colors.warning, icon: 'flag' },
        { label: 'Priority 5', value: 'priority5', color: colors.info, icon: 'flag' },
        { label: 'Priority 4', value: 'priority4', color: colors.textTertiary, icon: 'flag' }
    ];

    // Enhanced inbox/tag options with colors
    const inboxOptions = [
        { label: 'Inbox', value: 'Inbox', icon: 'mail-outline', color: colors.primary },
        { label: 'Work', value: 'Work', icon: 'briefcase-outline', color: colors.info },
        { label: 'Personal', value: 'Personal', icon: 'person-outline', color: colors.success },
        { label: 'Shopping', value: 'Shopping', icon: 'bag-outline', color: colors.warning },
        { label: 'Health', value: 'Health', icon: 'fitness-outline', color: colors.error },
        { label: 'Study', value: 'Study', icon: 'book-outline', color: colors.secondary },
        { label: 'Travel', value: 'Travel', icon: 'airplane-outline', color: colors.accent },
        { label: 'Finance', value: 'Finance', icon: 'card-outline', color: colors.tertiary }
    ];
    ...
    const sections = useMemo(() => {
        ...
        const allTasks = tasks?.filter(task => task.timestamp && !isNaN(new Date(task.timestamp).getTime())) || [];

        // Collect overdue (past unfinished tasks)
        const overdueTasks = allTasks
            .filter(task => {
                const taskDateStr = new Date(task.timestamp).toISOString().split('T')[0];
        ...
        allTasks.forEach(task => {
            const dateString = new Date(task.timestamp).toISOString().split('T')[0];
        ...
        if (overdueTasks.length > 0) {
            secs.push({
                title: 'Overdue',
                data: overdueTasks.map(task => ({ ...task, height: 80, isOverdue: true })),
            });
        }
    ...
    const renderTaskItem = (item) => {
        ...
        const priorityColor = item.isOverdue && !item.is_completed 
            ? colors.error 
            : item.is_completed 
                ? colors.success 
                    ...
                    >
                        <View
                            style={[
                                ...
                                item.is_completed && { backgroundColor: priorityColor },
                            ]}
                        >
                            {item.is_completed && (
                    ...
                    <View style={styles.taskDetails}>
                        ...
                        <View style={styles.taskMeta}>
                            <View style={styles.taskTimeContainer}>
                                <Ionicons name="calendar-outline" size={12} color={colors.error} />
                            ...
                            <View style={styles.inboxTag}>
                                ...
                                <Text style={styles.inboxText}>{item.tag || 'Inbox'}</Text>
                            </View>
                        </View>
                    </View>
                </View>
            ...
            <View style={styles.sectionHeader}>
                ...
                {isOverdue && (
                    <TouchableOpacity 
                        ...
                        onPress={() => Alert.alert('Reschedule', 'Reschedule overdue tasks functionality')}
                    >
                        <Text style={styles.rescheduleText}>Reschedule</Text>
                        <Ionicons name="chevron-up" size={16} color={colors.error} />
                    </TouchableOpacity>
                )}
            </View>
        );
    ...
    const getCurrentPriority = () => {
        return priorityOptions.find(p => p.value === newTask.priority) || { label: 'Priority', value: 'none', color: colors.textTertiary, icon: 'flag-outline' };
    };
    ...
    const getDateTimeDisplay = () => {
        ...
        return `${date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} at ${newTask.time}`;
    };
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerActions}>
                    <TouchableOpacity style={styles.headerButton}>
                        <Ionicons name="reorder-four" size={20} color={colors.textSecondary} />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.headerButton}>
      ...
      <TouchableWithoutFeedback onPress={() => {}}>
        ...
        >
          ...
          <View style={styles.inputContainer}>
            <View style={[styles.checkboxInner, { borderColor: colors.success }]} />
            <TextInput
              style={styles.input}
          ...
          <View style={styles.modalOptions}>
            ...
            <TouchableOpacity 
              style={[
                styles.optionButton,
                (newTask.isToday || newTask.date !== new Date().toISOString().split('T')[0]) && styles.optionButtonActive
              ]}
            ...
            >
              <Ionicons 
                ...
                size={16} 
                color={(newTask.isToday || newTask.date !== new Date().toISOString().split('T')[0]) ? colors.success : colors.textSecondary} 
              />
              <Text style={[
                styles.optionText,
                (newTask.isToday || newTask.date !== new Date().toISOString().split('T')[0]) && styles.optionTextActive
              ]}>
                {(newTask.isToday || newTask.date !== new Date().toISOString().split('T')[0]) ? getDateTimeDisplay() : 'Today'}
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity 
              style={[
                styles.optionButton,
                newTask.priority !== 'none' && styles.optionButtonActive
              ]}
            ...
            >
              <Ionicons 
                ...
                size={16} 
                color={newTask.priority !== 'none' ? getCurrentPriority().color : colors.textSecondary} 
              />
              <Text style={[
                styles.optionText,
                newTask.priority !== 'none' && styles.optionTextActive
              ]}>
                Priority
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity 
              style={[
                styles.optionButton,
                newTask.reminder && styles.optionButtonActive
              ]}
            ...
            >
              <Ionicons 
                ...
                size={16} 
                color={newTask.reminder ? colors.warning : colors.textSecondary} 
              />
              <Text style={[
                styles.optionText,
                newTask.reminder && styles.optionTextActive
              ]}>
                Reminders
              </Text>
            </TouchableOpacity>
          ...
          <TouchableOpacity 
            style={[
              ...
              newTask.tag !== 'Inbox' && styles.inboxSelectorActive
            ]}
          ...
          >
            <Ionicons 
              ...
              size={16} 
              color={getCurrentInbox().color || colors.textTertiary} 
            />
            <Text style={styles.inboxSelectorText}>{getCurrentInbox().label}</Text>
            <Ionicons 
              name={showInboxOptions ? "chevron-up" : "chevron-down"} 
              size={16} 
          ...
          {showInboxOptions && (
            <Animated.View 
              style={[
                ...
                { transform: [{ translateY: inboxSlideAnim }] } // Added animation
              ]}
            >
              <ScrollView 
              ...
              >
                {inboxOptions.map((inbox, index) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.inboxOption,
                      newTask.tag === inbox.value && styles.inboxOptionSelected
                    ]}
                  ...
                  >
                    <Ionicons 
                      name={inbox.icon} 
                      size={20} 
                      color={inbox.color || colors.textSecondary} 
                    />
                    <Text style={[
                      styles.inboxOptionText,
                      newTask.tag === inbox.value && styles.inboxOptionTextSelected
                    ]}>
                      {inbox.label}
                    </Text>
                  </TouchableOpacity>
